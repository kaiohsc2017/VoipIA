...
- Fase 2 (execução): 🔄 em andamento
  ...
  - ✅ **Fase 19 concluída em 2026-07-14** — ONDA 2, parte React (O2.5): hook `useAuthSession()`/`authSessionFromToken()` consolidando a leitura de token+role+perms repetida em `App.tsx`, `Operadoras.tsx`, `Cadastro0800.tsx`, `Linhas.tsx`; `decodeTokenPayload` exportado de `client.ts` e reusado por `Softphone.tsx`. Build de produção (`tsc -b && vite build`) limpo — **ainda não deployado/testado ao vivo nesta fase** (rebuild do container `frontend` pendente).
  - Próxima: ONDA 3 (God classes) — ver seção própria abaixo. Antes de avançar, considerar deployar/testar a fase 19 em produção.
