...
import type { AgentReportDto, AgentEvolutionSnapshot } from '../api/types';
...
/** Aba "Relatórios" — relatórios de performance por atendente (Fase 2 do Quality
 * Management, V39). Posse: supervisor só vê os relatórios que ele mesmo pediu;
 * ADMIN vê todos com coluna de solicitante. Cooldown de 5 dias úteis por par
 * (supervisor, atendente) — ADMIN isento. */
export function ReportsTab({ canWrite, isAdmin }: ReportsTabProps) {
  const [view, setView] = useState<'list' | 'detail' | 'history'>('list');
  const [reports, setReports] = useState<AgentReportDto[]>([]);
  ...
  const [selected, setSelected] = useState<AgentReportDto | null>(null);
  const [historyAgent, setHistoryAgent] = useState('');
  const [historyData, setHistoryData] = useState<AgentEvolutionSnapshot[]>([]);
  const [historyLoading, setHistoryLoading] = useState(false);
  const [historyError, setHistoryError] = useState('');

  const load = (p = 0) => {
    setLoading(true);
    api.get<PageResponse<AgentReportDto>>(`/insights/reports?page=${p}&size=20`)
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      api.get<{ nextAllowedAt?: string }>(`/insights/reports/agent/${encodeURIComponent(agentName)}/next-allowed`)
  ...
  const requestReport = async () => {
    ...
    setFormError('');
    try {
      await api.post('/insights/reports', { agentName: agentName.trim(), dateFrom, dateTo });
  ...
  const openDetail = (id: number) => {
    api.get<AgentReportDto>(`/insights/reports/${id}`)
  ...
  const exportPdf = async (report: AgentReportDto) => {
    try {
      const res = await api.get(`/insights/reports/${report.id}/pdf`, { responseType: 'blob' });
  ...
  const openHistory = (name: string) => {
    ...
    setHistoryError('');
    api.get<AgentEvolutionSnapshot[]>(`/insights/reports/agent/${encodeURIComponent(name)}/evolution`)
        ...
        {historyLoading ? (
          ...
          <div className="stat-card" style={{ padding: 20 }}>
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={notaSeries} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
                ...
                <Line type="monotone" dataKey="nota" name="Nota total" stroke="#007aff" strokeWidth={2} dot={{ r: 3 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        )}
