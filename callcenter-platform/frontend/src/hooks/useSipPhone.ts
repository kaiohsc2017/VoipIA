...
export function useSipPhone(enabled = true): SipPhoneApi {
  ...
  useEffect(() => {
    // D10-A: quando embutido no shell (isEmbedded=true no chamador), quem registra o UA SIP é
    // Softphone.tsx no shell — este hook não pode registrar um segundo UA no mesmo ramal.
    if (!enabled) return;
    let cancelled = false;
