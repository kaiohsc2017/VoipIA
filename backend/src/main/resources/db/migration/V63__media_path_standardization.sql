...
-- repositório (/opt/AsteriskIA/media/), git-ignorada (ver .gitignore).
--
...
UPDATE cc_recordings
   SET file_path = replace(file_path, '/opt/gravacoes/audio', '/opt/AsteriskIA/media/gravacao')
 WHERE file_path LIKE '/opt/gravacoes/audio%';

UPDATE cc_chat_sessions
   SET transcript_path = replace(transcript_path, '/opt/gravacoes/chat', '/opt/AsteriskIA/media/chat')
...
-- baseDir ATUAL (app.insights.upload-audio-path, agora /opt/AsteriskIA/media/sobdemanda) para
...
UPDATE call_audio_files
   SET wav_path = replace(wav_path, '/opt/audio_upload', '/opt/AsteriskIA/media/sobdemanda')
 WHERE source = 'upload'
