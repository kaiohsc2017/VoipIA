 ...
 *
 * GET    /api/v1/access-groups          → lista grupos com sua matriz de permissões
 * GET    /api/v1/access-groups/{id}     → busca um grupo
 * POST   /api/v1/access-groups          → cria grupo custom + matriz
 * PUT    /api/v1/access-groups/{id}     → atualiza nome/descrição/matriz
 * DELETE /api/v1/access-groups/{id}     → exclui (bloqueado para grupos de sistema)
 *
 * Protegido por ROLE_ADMIN puro em SecurityConfig — gerenciar grupos não usa
 * o próprio sistema de permissões granulares (evita o ovo-e-galinha de um
 * grupo customizado precisar de si mesmo pra existir).
 */
@Slf4j
@RestController
...
public class AccessGroupController {
    ...
    @PostMapping
    public ResponseEntity<?> createGroup(@Valid @RequestBody GroupRequest req, HttpServletRequest httpRequest) {
        if (groupRepo.findByName(req.name()).isPresent()) {
            return ResponseEntity.badRequest().body(new ErrorResponse("Já existe um grupo com esse nome: " + req.name()));
        }
        ...
        auditService.log(httpRequest, "ACCESS_GROUP_CREATE",
                "Grupo de acesso criado: " + saved.getName(), true);
    ...
    public ResponseEntity<?> updateGroup(@PathVariable Integer id,
        ...
        return groupRepo.findById(id)
                .map(group -> {
                    ...
                    auditService.log(httpRequest, "ACCESS_GROUP_UPDATE",
                            "Grupo de acesso '" + saved.getName() + "' atualizado", true);
                ...
                .map(group -> {
                    if (Boolean.TRUE.equals(group.getIsSystem())) {
                        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                                .body(new ErrorResponse("Grupos de sistema não podem ser excluídos: " + group.getName()));
                    }
                    ...
                    auditService.log(httpRequest, "ACCESS_GROUP_DELETE",
                            "Grupo de acesso excluído: " + group.getName(), true);
    ...
    private void savePermissions(AccessGroup group, List<PermissionEntry> entries) {
        ...
        List<AccessGroupPermission> perms = entries.stream()
                .map(e -> AccessGroupPermission.builder()
                        .group(group)
    ...
    private GroupResponse toResponse(AccessGroup group) {
        List<PermissionEntry> perms = permissionRepo.findByGroupId(group.getId()).stream()
                .map(p -> new PermissionEntry(p.getResourceKey(), p.getCanRead(), p.getCanWrite()))
                .toList();
        return new GroupResponse(group.getId(), group.getName(), group.getDescription(), group.getIsSystem(), perms);
    }
