 ...
 *
 * GET  /api/v1/uras/{uraId}/questions        — usado pelo agente Python (JiraCallFlow)
 * GET  /api/v1/uras/{uraId}/questions/all    — todas as perguntas (admin)
 * POST /api/v1/uras/{uraId}/questions        — adiciona nova pergunta
 * PUT  /api/v1/uras/{uraId}/questions/{id}   — atualiza pergunta
 * DELETE /api/v1/uras/{uraId}/questions/{id} — remove pergunta
 */
@RestController
...
public class UraQuestionController {
     ...
     */
    @GetMapping
        public ResponseEntity<List<UraQuestionResponse>> getActiveQuestions(@PathVariable Integer uraId) {
        List<UraQuestionResponse> questions = service.findActiveQuestions(uraId)
                .stream()
    ...
    @DeleteMapping("/{id}")
        public ResponseEntity<Void> deleteQuestion(@PathVariable Integer uraId, @PathVariable Integer id) {
        service.delete(id);
        return ResponseEntity.noContent().build();
    }
