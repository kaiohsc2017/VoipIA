...
public class UserController {
    ...
    private Set<BusinessUnit> resolveBusinessUnits(List<Integer> ids) {
        ...
        if (found.size() != Set.copyOf(ids).size()) {
            throw new IllegalArgumentException("Uma ou mais Unidades de Negócio informadas não existem.");
        }
        return new HashSet<>(found);
    }

    /**
     * Valida a regra "expiração de acesso XOR indeterminado": exatamente um
     * dos dois deve estar preenchido — indeterminado=true com data ausente,
     * ou uma data futura de até 60 dias com indeterminado=false.
     */
    private void validateAccessWindow(LocalDate accessExpiresAt, boolean accessIndeterminate) {
        if (accessIndeterminate) {
            if (accessExpiresAt != null) {
                throw new IllegalArgumentException("Acesso indeterminado não pode ter data de expiração.");
            }
            return;
        }
        if (accessExpiresAt == null) {
            throw new IllegalArgumentException("Informe a data de expiração do acesso ou marque acesso indeterminado.");
        }
        ...
        if (accessExpiresAt.isAfter(today.plusDays(MAX_ACCESS_DAYS))) {
            throw new IllegalArgumentException("A data de expiração não pode passar de " + MAX_ACCESS_DAYS + " dias a partir de hoje.");
        }
    }
        ...
        try {
            if (req.businessUnitIds() != null) {
                if (req.businessUnitIds().isEmpty()) {
                    return ResponseEntity.badRequest().body(new ErrorResponse("O usuário precisa de ao menos uma BU."));
                }
                user.setBusinessUnits(resolveBusinessUnits(req.businessUnitIds()));
            }
            if (req.accessIndeterminate() != null || req.accessExpiresAt() != null) {
                boolean indeterminate = req.accessIndeterminate() != null
    ...
    /**
     * Reset de MFA pelo administrador — usado quando o usuário perde acesso ao
     * app TOTP ou esquece a senha e não consegue completar o 2FA sozinho.
     * Diferente de TotpController.disable (self-service, exige código válido),
     * este endpoint não exige nenhuma prova do usuário-alvo — só ADMIN pode
     * chamar (SecurityConfig restringe /users/** de escrita a ROLE_ADMIN ou
    ...
    public ResponseEntity<?> resetTotp(@PathVariable Integer id, HttpServletRequest httpRequest) {
        return userRepo.findById(id)
                .map(user -> {
                    ...
                    userRepo.save(user);
                    log.info("MFA resetado pelo admin para usuário '{}'", user.getUsername());
                    auditService.log(httpRequest, "USER_TOTP_RESET",
                            "MFA resetado pelo admin para o usuário '" + user.getUsername() + "'", true);
    ...
    public record CreateUserRequest(
            ...
            String role,
            @NotEmpty(message = "Selecione ao menos uma Unidade de Negócio (BU)") List<Integer> businessUnitIds,
