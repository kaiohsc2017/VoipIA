 ...
 *
 * GET  /api/v1/ai/providers                       → lista provedores + se tem key
 * GET  /api/v1/ai/providers/{id}/models?cap=STT   → busca modelos via API do provedor
 * PUT  /api/v1/ai/providers/{id}/key              → salva API key
 * GET  /api/v1/ai/chain                           → lê todas as chains do banco
 * PUT  /api/v1/ai/chain/{capability}              → salva chain de uma capability
 * GET  /api/v1/ai/chain/active                    → endpoint interno para o ai-agent
 */
@Slf4j
@RestController
...
public class AiProviderController {
    ...
    public ResponseEntity<?> listProviders() {
        ...
        List<Map<String, Object>> result = AiProviderService.PROVIDERS.stream()
    ...
    public ResponseEntity<?> listModels(
            ...
            @RequestParam(defaultValue = "LLM") String cap) {
        try {
    ...
    public ResponseEntity<?> saveChain(
        ...
        if (!List.of("STT","LLM","TTS").contains(capability))
            return ResponseEntity.badRequest().body(Map.of("error", "capability inválida: " + capability));
    ...
    public ResponseEntity<?> getActiveChain() {
        ...
        for (String cap : List.of("STT","LLM","TTS")) {
            List<Map<String, String>> entries = service.getChain(cap).stream()
