 ...
 *
 * GET  /api/v1/config          → lista todas (secrets mascarados)
 * PUT  /api/v1/config/{key}    → atualiza uma chave
 * POST /api/v1/config/batch    → atualiza múltiplas chaves de uma vez (salvar seção)
 */
@Slf4j
@RestController
...
public class SystemConfigController {
    ...
    public ResponseEntity<?> update(
        ...
        if (MASK.equals(value)) {
            return ResponseEntity.ok(Map.of("message", "Nenhuma alteração — valor mascarado ignorado."));
        }

        configService.set(key, value, user);
        return ResponseEntity.ok(Map.of("message", "Configuração '" + key + "' atualizada. Efeito em até 60s."));
    }
    ...
    public ResponseEntity<?> updateBatch(
        ...
        if (updates == null || updates.isEmpty())
            return ResponseEntity.badRequest().body(Map.of("message", "Nenhuma configuração enviada."));
        ...
        return ResponseEntity.ok(Map.of(
                "message", filtered.size() + " configuração(ões) atualizada(s). Efeito em até 60s.",
                "keys", filtered.keySet()
        ));
    }
