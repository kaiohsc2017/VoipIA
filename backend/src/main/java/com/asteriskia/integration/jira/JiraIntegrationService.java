 ...
 *
 * Autenticação: Basic Auth com e-mail + API Token (Base64).
...
public class JiraIntegrationService {
     ...
     *
     * @param fields Mapa de campos (jira_field_key → valor string).
     *               Campos iniciados com "customfield_" são incluídos diretamente.
    ...
    public String createIssue(Map<String, String> fields) {
        ...
        try {
            ...
            Map<String, Object> body = Map.of(
                "fields", buildIssueFields(fields, projectKey, issueType)
            );
            ...
            Map<?, ?> response = webClientBuilder.build()
                    ...
                    // X-Atlassian-Token: no-check — necessário para contornar XSRF em algumas instâncias
                    ...
                    .onStatus(HttpStatusCode::isError, clientResponse ->
                        clientResponse.bodyToMono(String.class)
                            .flatMap(errorBody -> {
                                log.error("Jira API erro {}: {}", clientResponse.statusCode(), errorBody);
                    ...
                    .onErrorResume(e -> {
                        log.error("Erro ao criar issue no Jira: {}", e.getMessage());
                        return Mono.empty();
                    })
                    .block();
    ...
    /**
     * Valida as credenciais chamando GET /rest/api/3/myself.
     * Retorna o displayName do usuário autenticado, ou lança exceção em caso de falha.
     */
                ...
                .onStatus(HttpStatusCode::isError, clientResponse ->
                    clientResponse.bodyToMono(String.class)
                        .flatMap(body -> Mono.error(
                            new RuntimeException("HTTP " + clientResponse.statusCode() + ": " + body)))
                )
                .bodyToMono(Map.class)
                .timeout(JIRA_TIMEOUT)
                .block();
     ...
     *
     * Campos customizados cujos valores são strings simples são enviados
     * diretamente. Para campos select/multi-select, o chamador deve prefixar
     * a chave com "customfield_" e passar o valor como "option:<label>" para
     * que seja convertido para {"value": "<label>"}.
     */
    ...
    private String buildSummary(Map<String, String> fields) {
        ...
        return cliente.isBlank()
            ? "Chamado URA — AsteriskIA"
            : "Chamado URA — " + cliente;
    }

    /**
     * Constrói um documento ADF (Atlassian Document Format) de parágrafo simples.
     * A API v3 do Jira rejeita description como string plana — exige ADF.
     *
    ...
    /**
     * Monta o header Basic Auth conforme a documentação da API v3.
     * Formato: Base64(email:apiToken) — não usar usuário/senha, apenas API Token.
     */
